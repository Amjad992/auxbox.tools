// Tailwind v3.4 default palette (a subset that excludes pure black/white aliases).
// Source: https://tailwindcss.com/docs/customizing-colors
//
// Each entry is [name, hex]. We resolve the nearest Tailwind name to an
...
export function nearestTailwind({r, g, b}) {
  ...
  for (const {name, rgb} of TAILWIND_RGB) {
    const dr = r - rgb.r;
    const dg = g - rgb.g;
    const db = b - rgb.b;
...
export const TAILWIND_PALETTE_SIZE = TAILWIND_RGB.length;
